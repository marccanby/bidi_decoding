...
def save_checkpoint(cfg: CheckpointConfig, trainer, epoch_itr, val_loss):
    ...
    met = cfg.best_checkpoint_metric
    best = trainer.task.getBestMetric(met, val_loss, do_min=not cfg.maximize_best_checkpoint_metric)
    ...
    checkpoints = [cfg.save_dir + '/checkpoint_curr.pt']
    if val_loss == best:
        checkpoints.append(cfg.save_dir + '/checkpoint_best.pt')
    if len(checkpoints) > 0:
        #trainer.save_checkpoint(checkpoints[0], extra_state)
        for cp in checkpoints: trainer.save_checkpoint(cp, extra_state)
