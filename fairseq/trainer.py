        ...
        if bexists:
            ...
            try:
                for name, param in state['model'].items():
                    sdict = self.model.l2r_model.state_dict() if dir == 'l2r' else self.model.r2l_model.state_dict()
                    if name not in sdict:
                        assert False, 'should not happen'
                        # assert name == 'decoder.output_projection.weight'
                        # sdicthere = self.model.output_projection_l2r.state_dict() if dir == 'l2r' else self.model.output_projection_r2l.state_dict()
                        # sdicthere['weight'] = param
                        continue
                    if isinstance(param, torch.nn.Parameter):
                        # backwards compatibility for serialized parameters
                        param = param.data
                    sdict[name].copy_(param)
