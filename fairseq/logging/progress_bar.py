...
class WandBProgressBarWrapper(BaseProgressBar):
    ...
    def __init__(self, wrapped_bar, wandb_project, run_name=None):
        self.wrapped_bar = wrapped_bar
        if os.path.isdir('/scratch/marcec2/wandb'):
            self.wandb_path = '/scratch/marcec2/wandb'
        elif os.path.isdir('/home/marccanby/wandb'):
            self.wandb_path = '/home/marccanby/wandb'
        if wandb is None:
            logger.warning("wandb not found, pip install wandb")
            return

        # The following 2 lines don't work for some reason. Instead, just leaving all folders with wandb (see below)
        #    and periodically deleting wandb from commadn line
        # if os.path.exists(self.wandb_path): shutil.rmtree(self.wandb_path)
        # os.mkdir(self.wandb_path)

        # reinit=False to ensure if wandb.init() is called multiple times
        # within one process it still references the same run
        wandb.init(project=wandb_project, reinit=False, name=run_name, dir=self.wandb_path)
    ...
    def _log_to_wandb(self, stats, tag=None, step=None):
        ...
        if random.random() < 0.1: # Only do this about 10% of the times logging is called
            nowtime = datetime.datetime.now()
            timetodelete = nowtime - datetime.timedelta(hours=0, minutes=3) # Delete everything older than 3 mins
            lst = sorted(os.listdir(self.wandb_path + '/wandb/')) # All of the logs in wandb folder

            check_all_files_instead_of_just_do_first_wandb_found = True
            found_inagural = False # Can't delete the original wandb log for this run
            for x in lst:
                if x[0:3] != 'run': continue
                splt = x.split('-')[1]
                dt, tm = splt.split('_')[0], splt.split('_')[1]
                assert len(dt) == 8
                assert len(tm) == 6
                stamp = datetime.datetime(int(dt[0:4]), int(dt[4:6]), int(dt[6:8]), int(tm[:2]), int(tm[2:4]), int(tm[4:6]), 0)
                if stamp <= timetodelete:
                    path = self.wandb_path + '/wandb/' + x
                    if check_all_files_instead_of_just_do_first_wandb_found: assert not found_inagural
                    try:
                        if not found_inagural:
                            lst2 = os.listdir(path)
                            this_is_inaugural = False
                            for y in lst2:
                                if '.wandb' in y:
                                    if not check_all_files_instead_of_just_do_first_wandb_found: found_inagural = True
                                    this_is_inaugural = True
                                    break
                        else:
                            assert not check_all_files_instead_of_just_do_first_wandb_found
                            this_is_inaugural = False
                        if not this_is_inaugural:
                            shutil.rmtree(path)
                    except:
                        print("<WANDB> Unable to deal with ", path)
