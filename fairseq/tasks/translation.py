...
class TranslationTask(FairseqTask):
    ...
    def __init__(self, cfg: TranslationConfig, src_dict, tgt_dict):
        ...
        self.best_metrics = {} # My dictionary
    ...
    def valid_step(self, sample, model, criterion):
        ...
        if self.cfg.eval_bleu:
            bleu, stats = self._inference_with_bleu(self.sequence_generator, sample, model)
    ...
    def reduce_metrics(self, logging_outputs, criterion):
        ...
        if self.cfg.eval_bleu:
            ...
            if '_num_full_l2r' in logging_outputs[0]:
                metrics.log_scalar("_num_full_l2r", sum_logs("_num_full_l2r"))
                metrics.log_scalar("_num_full_r2l", sum_logs("_num_full_r2l"))
                metrics.log_scalar("_num_neither", sum_logs("_num_neither"))
                metrics.log_scalar("_num_left_moves", sum_logs("_num_left_moves"))
                metrics.log_scalar("_num_right_moves", sum_logs("_num_right_moves"))
                metrics.log_scalar("_nsentences", sum_logs("nsentences"))

                def perc_full_l2r(meters):
                    acc = meters['_num_full_l2r'].sum / meters['_nsentences'].sum * 100
                    return acc.item() if type(acc) == torch.Tensor else acc

                def perc_full_r2l(meters):
                    acc = meters['_num_full_r2l'].sum / meters['_nsentences'].sum * 100
                    return acc.item() if type(acc) == torch.Tensor else acc

                def perc_neither(meters):
                    acc = meters['_num_neither'].sum / meters['_nsentences'].sum * 100
                    return acc.item() if type(acc) == torch.Tensor else acc

                def avg_perc_left_moves(meters):
                    acc = meters['_num_left_moves'].sum / meters['_nsentences'].sum * 100
                    return acc.item() if type(acc) == torch.Tensor else acc

                def avg_perc_right_moves(meters):
                    acc = meters['_num_right_moves'].sum / meters['_nsentences'].sum * 100
                    return acc.item() if type(acc) == torch.Tensor else acc
            ...
            if max(totals) > 0:
                ...
                def compute_bleu(meters):
                    ...
                    if "smooth_method" in fn_sig:
                        ...
                        smooth = {"smooth": "exp"}
                    sys_len, ref_len = meters["_bleu_sys_len"].sum, meters["_bleu_ref_len"].sum
                    assert sys_len % 1 == 0 and ref_len % 1 == 0 # making sure both ints, even if t'e floats
                    sys_len, ref_len = int(sys_len), int(ref_len)
                    bleu = comp_bleu(
    ...
    def _inference_with_bleu(self, generator, sample, model):
        ...
        for i in range(len(gen_out)):
            ...
            if 'directions' in gen_out[i][0]:
                directions = gen_out[i][0]['directions'].tolist()
        ...
        if self.cfg.eval_bleu_print_samples:
            ...
            if len(dirs) > 0: logger.info("Example direction: " + str(dirs[0]))
        if len(dirs) > 0:
            stats = getOrderingStats(dirs)
        else: stats = None

        if self.cfg.eval_tokenized_bleu:
            return sacrebleu.corpus_bleu(hyps, [refs], tokenize="none"), stats
        else:
            return sacrebleu.corpus_bleu(hyps, [refs]), stats
...
# straight up copied from morph task
